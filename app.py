...
def vector_embedding():
    ...
    st.session_state.final_documents = st.session_state.text_splitter.split_documents(st.session_state[:10])       # 4 (First 10 documents just for testing
    st.session_state.vectors = FAISS.from_documents(st.session_state.final_documents, st.session_state.embeddings) # 5

prmpt1 = st.text_input("Enter your Question (About the Documents)")
